...
export default function AdminDashboard() {
  ...
  // Typed text effect
  const message =
    "Control. Security. Stability. Build a platform that empowers everyone.";
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-gray-100 via-gray-200 to-red-100 text-black flex flex-col">
      ...
      flex items-center justify-between px-10 fixed top-0 z-50 shadow-md">
        ...
        <div className="flex-1 flex justify-center">
          <div className="flex gap-10 text-lg font-semibold">
            <button onClick={() => router.push("/6-admin/profile")} className="hover:text-red-700">
              Profile
            </button>
            <button onClick={() => router.push("/6-admin/users")} className="hover:text-red-700">
              Users
            </button>
            <button onClick={() => router.push("/6-admin/roles")} className="hover:text-red-700">
              Roles
            </button>
            <button onClick={() => router.push("/6-admin/logs")} className="hover:text-red-700">
              Logs
            </button>
            <button onClick={() => router.push("/6-admin/settings")} className="hover:text-red-700">
              Settings
            </button>
            <button onClick={() => router.push("/6-admin/system")} className="hover:text-red-700">
              System
            </button>
          </div>
        </div>
          ...
          {showProfileMenu && (
            <div className="absolute right-0 top-16 bg-white text-black shadow-lg rounded-lg w-48 py-2 z-50">
              ...
              <div
                ...
                onClick={() => router.push("/6-admin/profile")}
              >
                View Profile
              </div>
      ...
      <section className="flex flex-col items-center justify-center flex-1 text-center px-4">
        ...
        {/* TEXT MOTIVAȚIONAL */}
        <h1 className="text-4xl font-bold tracking-wide text-red-900">
          {typedText}
