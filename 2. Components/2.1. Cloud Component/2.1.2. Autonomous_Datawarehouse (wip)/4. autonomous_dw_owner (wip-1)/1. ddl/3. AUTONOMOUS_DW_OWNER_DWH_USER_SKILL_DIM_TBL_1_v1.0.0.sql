...
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.prerequisite_knowledge_score IS ''The score of the prerequisite knowledge that a user should know before learn this skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.learning_difficulty_score IS ''The score of the learning difficulty of that skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.implementation_difficulty_score IS ''The score of the implementation difficulty of that skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.cross_platform_applicability_score IS ''The score of the cross platform applicability of that skill''';
...
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.version_release_date IS ''The release date of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.version_end_of_life IS ''The end of life date of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.version_age IS ''The age of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.developer_popularity_score IS ''The score of the developer popularity of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.community_support_score IS ''The score of the community support of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.industry_usage_score IS ''The score of the industry usage of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.knowledge_score IS ''The score of the knowledge of the skill version''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_user_skill_dim.version_skill_rating IS ''The rating of the version skills''';
...
--CREATE TRIGGER FOR POPULATING version_age COLUMN
--DELETE TRIGGER IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_triggers
WHERE trigger_name = 'TRG_USER_VERSION_AGE_CALC';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER autonomous_dw_owner.trg_user_version_age_calc';
END IF;
--CREATE TRIGGER
v_sql := '  CREATE OR REPLACE TRIGGER trg_user_version_age_calc
BEFORE INSERT OR UPDATE ON autonomous_dw_owner.dwh_user_skill_dim
FOR EACH ROW
BEGIN
    :NEW.version_age :=
...
WHERE trigger_name = 'TRG_USER_VERSION_AGE_CALC';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The TRG_USER_VERSION_AGE_CALC trigger wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[7.] The TRG_USER_VERSION_AGE_CALC trigger for populating version_age column was created.');
