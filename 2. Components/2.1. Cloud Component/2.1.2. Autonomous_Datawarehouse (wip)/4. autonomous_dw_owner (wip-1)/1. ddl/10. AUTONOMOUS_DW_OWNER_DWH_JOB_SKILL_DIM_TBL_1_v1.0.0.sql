--AUTONOMOUS_DW_OWNER_DWH_JOB_SKILL_DIM_TBL_1_v1.0.0
--"DWH_JOB_SKILL_DIM TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND table_name = 'DWH_JOB_SKILL_DIM'
AND tablespace_name = 'DATA';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TABLE autonomous_dw_owner.dwh_job_skill_dim CASCADE CONSTRAINTS';
END IF;


--CREATE UTILIZATORI TABLE;
v_sql := q'[
        CREATE TABLE autonomous_dw_owner.dwh_job_skill_dim (
...
EXECUTE IMMEDIATE 'GRANT SELECT ON autonomous_dw_owner.dwh_job_skill_dim TO autonomous_dw_out_owner';
...
AND tablespace_name = 'DATA';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The DWH_JOB_SKILL_DIM table wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The DWH_JOB_SKILL_DIM table was created.');
...
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_job_skill_dim.technology_type_rating IS ''The rating of the technology type''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_job_skill_dim.overall_skill_rating IS ''The rating of the skill over version,technology and technology_type''';
...
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_job_skill_dim.valid_to IS ''Technical Column - The timestamp indicating when the record stops being effective in the data warehouse.''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_job_skill_dim.source_system IS ''Technical Column - The source system of the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_dw_owner.dwh_job_skill_dim.deleted_flag IS ''Technical Column - The flag indicating if the record is deleted or not''';

--CREATE SEQUENCE SEQ_TECHNOLOGY_TYPE_ID FOR PRIMARY KEY
--DELETE SEQUENCE IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_sequences
WHERE sequence_name = 'SEQ_JOB_SKILL_KEY';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP SEQUENCE autonomous_dw_owner.seq_job_skill_key';
END IF;
--CREATE SEQUENCE
 EXECUTE IMMEDIATE '
    CREATE SEQUENCE seq_job_skill_key
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE
...
WHERE sequence_name = 'SEQ_JOB_SKILL_KEY';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The SEQ_JOB_SKILL_KEY sequence wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[4.] The SEQ_JOB_SKILL_KEY sequence for primary key was created.');

--CREATE TRIGGER FOR PRIMARY KEY;
--DELETE TRIGGER IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_triggers
WHERE trigger_name = 'TRG_JOB_SKILL_KEY_PK';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER autonomous_dw_owner.trg_job_skill_key_pk';
END IF;
--CREATE TRIGGER
v_sql := 'CREATE OR REPLACE TRIGGER trg_job_skill_key_pk
          BEFORE INSERT ON autonomous_dw_owner.dwh_job_skill_dim
          FOR EACH ROW
          WHEN (NEW.job_skill_key IS NULL)
          BEGIN
             SELECT seq_job_skill_key.NEXTVAL INTO :NEW.job_skill_key FROM dual;
          END;';          
EXECUTE IMMEDIATE v_sql;
...
WHERE trigger_name = 'TRG_JOB_SKILL_KEY_PK';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The TRG_JOB_SKILL_KEY_PK trigger wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[5.] The TRG_JOB_SKILL_KEY_PK trigger for primary key was created.');
--CREATE TRIGGER FOR TECHNICAL COLUMNS
--DELETE TRIGGER IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_triggers
WHERE trigger_name = 'TRG_DWH_JOB_SKILL_DIM_TECH_COL';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER autonomous_dw_owner.trg_dwh_job_skill_dim_tech_col';
END IF;
--CREATE TRIGGER
v_sql := '  CREATE OR REPLACE TRIGGER trg_dwh_job_skill_dim_tech_col
        ...
        :NEW.created_by := ''ETL_JOB_SKILL_DAILY_PROCESS'';

        -- update metadata
        :NEW.last_update_date := :NEW.creation_date;
        :NEW.last_updated_by := ''ETL_JOB_SKILL_DAILY_PROCESS'';
...
WHERE trigger_name = 'TRG_DWH_JOB_SKILL_DIM_TECH_COL';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The TRG_DWH_JOB_SKILL_DIM_TECH_COL trigger wasnt created properly.');
END IF;
