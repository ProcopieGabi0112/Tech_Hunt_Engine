--AUTONOMOUS_DW_OWNER_DWH_JOB_LOCATION_DIM_JOB_1_v1.0.0
--"DWH_JOB_LOCATION_DIM JOB"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND object_name ='PRC_ETL_JOB_LOCATION_INITIAL_LOAD';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP PROCEDURE prc_etl_job_location_initial_load;';
END IF;
-- CREATE PROCEDURE FOR INITIAL LOAD
v_sql := q'[
             CREATE OR REPLACE PROCEDURE prc_etl_job_location_initial_load
             AS
                v_count             NUMBER;
    ...
    VALUES (
        'ETL_JOB_LOCATION_INITIAL_LOAD_PROCESS',
    ...
    MERGE INTO autonomous_dw_owner.dwh_job_location_dim d
    USING (
            SELECT DISTINCT
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The PRC_ETL_JOB_LOCATION_INITIAL_LOAD procedure wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The PRC_ETL_JOB_LOCATION_INITIAL_LOAD procedure was created.');

EXECUTE IMMEDIATE 'BEGIN admin.prc_etl_job_location_initial_load; END;';

DBMS_OUTPUT.PUT_LINE('[4.] The PRC_ETL_JOB_LOCATION_INITIAL_LOAD is running.');
...
WHERE owner = 'ADMIN'
AND object_type = 'PROCEDURE'
AND object_name ='PRC_ETL_JOB_LOCATION_DAILY';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP PROCEDURE prc_etl_job_location_daily;';
END IF;

-- CREATE PROCEDURE 
v_sql := q'[
              CREATE OR REPLACE PROCEDURE prc_etl_job_location_daily
             AS
                v_count             NUMBER;
                v_notification_id   VARCHAR2(200);
                v_error             VARCHAR2(4000);
    ...
    VALUES (
        'ETL_JOB_LOCATION_DAILY_PROCESS',
        TO_CHAR(SYSDATE,'YYYY-MM-DD'),
        'DAILY',
        CURRENT_TIMESTAMP,
        'IN_PROGRESS',
    ...
    UPDATE autonomous_dw_owner.dwh_job_location_dim d
    SET
           d.deleted_flag = 'Y',
           d.last_update_date = CURRENT_TIMESTAMP
    WHERE NOT EXISTS (
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The PRC_ETL_JOB_LOCATION_DAILY procedure wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[5.] The PRC_ETL_JOB_LOCATION_DAILY procedure was created.');

SELECT COUNT(*) INTO v_count
FROM dba_scheduler_jobs
WHERE owner = 'ADMIN'
AND job_name = 'JOB_ETL_JOB_LOCATION_DAILY_PROCESS';

IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB (
        job_name => 'JOB_ETL_JOB_LOCATION_DAILY_PROCESS',
        force    => TRUE
    );
END IF;
...
DBMS_SCHEDULER.CREATE_JOB (
        ...
        job_action      => 'ADMIN.PRC_ETL_JOB_LOCATION_DAILY',
        ...
        comments        => 'Daily ETL process for DWH_JOB_LOCATION_DIM'

    );

    
DBMS_OUTPUT.PUT_LINE('[6.] The PRC_ETL_JOB_LOCATION_DAILY scheduled job was created.');
