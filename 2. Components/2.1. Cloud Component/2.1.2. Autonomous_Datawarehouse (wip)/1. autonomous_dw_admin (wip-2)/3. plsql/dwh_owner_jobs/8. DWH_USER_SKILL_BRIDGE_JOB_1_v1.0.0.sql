--AUTONOMOUS_DW_OWNER_DWH_USER_SKILL_BRIDGE_JOB_1_v1.0.0
--"DWH_USER_SKILL_BRIDGE JOB"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
v_sql := q'[
    ...
    MERGE INTO autonomous_dw_owner.dwh_user_skill_bridge d
    USING (
       SELECT DISTINCT 
              skill_dim.user_skill_key,
              us_dim.user_key,
              us.proficiency_level,
              us.experience_months,
              us.last_used_date,
              us.confidence_score
        FROM autonomous_dw_landing_owner.dwh_user_skill us
        JOIN autonomous_dw_owner.dwh_user_skill_dim skill_dim ON us.skill_code = skill_dim.skill_code
        JOIN autonomous_dw_owner.dwh_user_dim us_dim ON us.user_id = us_dim.user_id
       
               ) s
    ON (d.user_skill_key = s.user_skill_key 
    AND d.user_key = s.user_key)

    WHEN MATCHED THEN UPDATE SET

        d.proficiency_level   = s.proficiency_level,
        d.experience_months   = s.experience_months,
        d.last_used_date      = s.last_used_date,
        d.confidence_score    = s.confidence_score,
        d.deleted_flag        = 'N',
...
v_sql := q'[
  ...
  INSERT INTO autonomous_dw_tech_owner.dwh_processes_notif (
    ...
    FROM dual
      WHERE EXISTS ( SELECT 1 
                   FROM autonomous_dw_tech_owner.dwh_processes_notif
                   WHERE process_name = 'ETL_SKILL_DAILY_PROCESS' 
                   AND TO_DATE(process_date, 'YYYY-MM-DD') = TRUNC(SYSDATE)
...
VALUES (
    ...
    UPDATE autonomous_dw_owner.dwh_user_skill_bridge d
    SET
           d.deleted_flag = 'Y',
           d.last_update_date = CURRENT_TIMESTAMP
    WHERE NOT EXISTS (
                       ...
                       FROM autonomous_dw_landing_owner.dwh_user_skill us
                            JOIN autonomous_dw_owner.dwh_user_skill_dim skill_dim ON us.skill_code = skill_dim.skill_code
                            JOIN autonomous_dw_owner.dwh_user_dim us_dim ON us.user_id = us_dim.user_id
                       WHERE skill_dim.user_skill_key  = d.user_skill_key 
...
DBMS_SCHEDULER.CREATE_JOB (
        ...
        comments        => 'Daily ETL process for DWH_USER_SKILL_BRIDGE'
