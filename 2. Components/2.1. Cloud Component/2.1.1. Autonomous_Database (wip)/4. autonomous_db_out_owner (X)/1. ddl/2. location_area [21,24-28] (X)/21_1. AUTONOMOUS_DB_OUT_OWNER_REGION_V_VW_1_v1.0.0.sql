--AUTONOMOUS_DB_OUT_OWNER_REGION_V_VW_1_v1.0.0
--"REGION_V VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'REGION_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.region_v CASCADE CONSTRAINTS';
END IF;
--CREATE TECHNOLOGY_TYPE TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.region_v AS
SELECT 
region_id,
name,
code,
...
last_synced_at,
deleted_flag
FROM autonomous_db_owner.region;
]';
...
AND view_name = 'REGION_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The REGION_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The REGION_V view was created.');
