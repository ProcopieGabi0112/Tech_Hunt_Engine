...
AND object_name ='PRC_ETL_INSTITUTION_LOCATION_INITIAL_LOAD';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP PROCEDURE prc_etl_institution_location_initial_load;';
END IF;
-- CREATE PROCEDURE FOR INITIAL LOAD
v_sql := q'[
             CREATE OR REPLACE PROCEDURE prc_etl_institution_location_initial_load
             AS
                v_count             NUMBER;
    ...
    VALUES (
        'ETL_INSTITUTION_LOCATION_INITIAL_LOAD_PROCESS',
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The PRC_ETL_INSTITUTION_LOCATION_INITIAL_LOAD procedure wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The PRC_ETL_INSTITUTION_LOCATION_INITIAL_LOAD procedure was created.');

EXECUTE IMMEDIATE 'BEGIN admin.prc_etl_institution_location_initial_load; END;';

DBMS_OUTPUT.PUT_LINE('[4.] The PRC_ETL_INSTITUTION_LOCATION_INITIAL_LOAD is running.');
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The PRC_ETL_INSTITUTION_LOCATION_DAILY procedure wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[5.] The PRC_ETL_INSTITUTION_LOCATION_DAILY procedure was created.');
...
DBMS_SCHEDULER.CREATE_JOB (
        ...
        job_action      => 'ADMIN.PRC_ETL_INSTITUTION_LOCATION_DAILY',
...
DBMS_OUTPUT.PUT_LINE('[6.] The PRC_ETL_INSTITUTION_LOCATION_DAILY scheduled job was created.');
