--AUTONOMOUS_DW_LANDING_OWNER_DWH_ROLE_TBL_1_v1.0.0
--"DWH_ROLE TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND table_name = 'DWH_ROLE'
AND tablespace_name = 'DATA';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TABLE autonomous_dw_landing_owner.dwh_role CASCADE CONSTRAINTS';
END IF;
--CREATE ROLE TABLE;
v_sql := q'[
        CREATE TABLE autonomous_dw_landing_owner.dwh_role (
...
EXECUTE IMMEDIATE 'GRANT SELECT ON autonomous_dw_landing_owner.dwh_role TO autonomous_dw_owner';
...
AND tablespace_name = 'DATA';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The DWH_ROLE table wasnt created properly.');
END IF;
