--AUTONOMOUS_DW_OUT_OWNER_DWH_JOB_LOCATION_DIM_V_VW_1_v1.0.0
--"DWH_JOB_LOCATION_DIM_V TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_dw_out_owner.dwh_job_location_dim_v CASCADE CONSTRAINTS';
END IF;
--CREATE DWH_CERTIFICATION_BRIDGE_V TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_dw_out_owner.dwh_job_location_dim_v AS
SELECT 
job_location_key,
...
source_system,
deleted_flag
FROM autonomous_dw_owner.dwh_job_location_dim;
]';
...
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The DWH_JOB_LOCATION_DIM_V view was created.');
