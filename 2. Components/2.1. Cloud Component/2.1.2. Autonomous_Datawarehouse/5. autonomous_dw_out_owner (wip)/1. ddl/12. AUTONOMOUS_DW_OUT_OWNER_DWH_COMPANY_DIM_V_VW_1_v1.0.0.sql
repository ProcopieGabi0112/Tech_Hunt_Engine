--AUTONOMOUS_DW_OUT_OWNER_DWH_COMPANY_DIM_V_VW_1_v1.0.0
--"DWH_COMPANY_DIM_V TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'DWH_COMPANY_DIM_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_dw_out_owner.dwh_company_dim_v CASCADE CONSTRAINTS';
END IF;
--CREATE DWH_CERTIFICATION_BRIDGE_V TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_dw_out_owner.dwh_company_dim_v AS
SELECT 
company_key,
company_code,
company_name,
...
source_system,
deleted_flag
FROM autonomous_dw_owner.dwh_company_dim;
]';
...
AND view_name = 'DWH_COMPANY_DIM_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The DWH_COMPANY_DIM_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The DWH_COMPANY_DIM_V view was created.');
