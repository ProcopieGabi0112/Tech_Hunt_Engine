--AUTONOMOUS_DW_OUT_OWNER_DWH_JOB_SKILL_BRIDGE_V_VW_1_v1.0.0
--"DWH_JOB_SKILL_BRIDGE_V TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'DWH_JOB_SKILL_BRIDGE_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_dw_out_owner.dwh_job_skill_bridge_v CASCADE CONSTRAINTS';
END IF;
--CREATE DWH_SKILL_BRIDGE_V TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_dw_out_owner.dwh_job_skill_bridge_v AS
SELECT 
job_key,
job_skill_key,
required_level,
...
source_system,
deleted_flag
FROM autonomous_dw_owner.dwh_job_skill_bridge;
]';
...
AND view_name = 'DWH_JOB_SKILL_BRIDGE_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The DWH_JOB_SKILL_BRIDGE_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The DWH_JOB_SKILL_BRIDGE_V view was created.');
