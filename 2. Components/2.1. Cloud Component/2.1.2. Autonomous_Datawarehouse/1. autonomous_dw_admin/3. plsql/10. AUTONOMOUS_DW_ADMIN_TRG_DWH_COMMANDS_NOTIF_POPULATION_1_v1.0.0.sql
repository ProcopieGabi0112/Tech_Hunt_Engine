CREATE OR REPLACE TRIGGER trg_dwh_commands_notif_on_<TABLE_NAME>
AFTER INSERT OR UPDATE OR DELETE ON autonomous_dw_owner.<TABLE_NAME>
FOR EACH ROW
DECLARE
...
-------------------------------------------------------------------
  ...
  INSERT INTO autonomous_dw_tech_owner.dwh_commands_notif (
    comm_audit_key,
    user_name,
    command_timestamp,
    sql_command,
    ...
    'AUTONOMOUS_DW_OWNER',                                  -- schema
    'AUTONOMOUS_DATAWAREHOUSE_SYSTEM',                      -- created_by
    'dw_env'                                                -- source_system
  );
END;
/
