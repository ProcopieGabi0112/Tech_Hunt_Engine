-- AUTONOMOUS_DW_ADMIN_JOB_DWH_RESOURCES_NOTIF_CLEAN_UP_1_v1.0.0
--"CLEAN UP JOB FOR DWH_RESOURCES_NOTIF TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
WHERE job_name = 'JOB_CLEAN_UP_DWH_RESOURCES_NOTIF';
IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB('JOB_CLEAN_UP_DWH_RESOURCES_NOTIF', FORCE => TRUE);
END IF;
...
BEGIN
  DBMS_SCHEDULER.create_job (
    job_name        => 'JOB_CLEAN_UP_DWH_RESOURCES_NOTIF',
    ...
    job_action      => q'{
      ...
      BEGIN
        ...
        (process_name, 
         ...
         'DWH_RESOURCES_NOTIF_CLEAN_UP_PROCESS', 
        ...
        -- 4. Cleanup logic 
        -------------------- 
        DELETE FROM autonomous_dw_tech_owner.dwh_resources_notif 
        WHERE creation_date < SYSTIMESTAMP - INTERVAL '3' DAY;
...
END;
    ...
    enabled         => TRUE,
    comments        => 'Deletes records older than 3 days from DWH_RESOURCES_NOTIF table'
  );
END;
]';
...
SELECT COUNT(*) INTO v_count
FROM user_scheduler_jobs
WHERE job_name = 'JOB_CLEAN_DWH_RESOURCES_NOTIF';

IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The JOB_CLEAN_UP_DWH_RESOURCES_NOTIF job wasnt created properly.');
END IF;

DBMS_OUTPUT.PUT_LINE('[3.] The JOB_CLEAN_UP_DWH_RESOURCES_NOTIF job was created.');
