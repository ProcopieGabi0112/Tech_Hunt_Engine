-- AUTONOMOUS_DW_ADMIN_JOB_DWH_RESOURCES_NOTIF_POPULATION_1_v1.0.0
-- "POPULATION JOB FOR DWH_RESOURCES_NOTIF TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
  v_sql   CLOB;
BEGIN
  ...
  WHERE job_name = 'JOB_POPULATION_DWH_RESOURCES_NOTIF';

  IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB('JOB_POPULATION_DWH_RESOURCES_NOTIF', FORCE => TRUE);
  END IF;
  -----------------------------------------------
  -- [3.] CREATE OR REPLACE PROCEDURE JOB LOGIC
  -----------------------------------------------
EXECUTE IMMEDIATE q'[
    CREATE OR REPLACE PROCEDURE admin.proc_population_dwh_resources_notif AS
    ...
    BEGIN
      ...
      INSERT INTO autonomous_dw_tech_owner.dwh_processes_notif (
        ...
        'DWH_RESOURCES_NOTIF_POPULATION_PROCESS',
      ...
      --------------------------------------------------------------
      FOR r IN c_ts LOOP
        INSERT INTO autonomous_dw_tech_owner.dwh_resources_notif (
  ...
  ---------------------------------------
   DBMS_SCHEDULER.CREATE_JOB (
    job_name        => 'JOB_POPULATION_DWH_RESOURCES_NOTIF',
    job_type        => 'STORED_PROCEDURE',
    job_action      => 'proc_population_dwh_resources_notif',
    start_date      => SYSTIMESTAMP,
    repeat_interval => 'FREQ=HOURLY; INTERVAL=1',
    enabled         => TRUE,
    comments        => 'Populate the DWH_RESOURCES_NOTIF table'
  );
  ---------------------
  -- [5.] VERIFY JOB
  ---------------------
  ...
  IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The JOB_POPULATION_DWH_RESOURCES_NOTIF job wasn''t created properly.');
  END IF;

  DBMS_OUTPUT.PUT_LINE('[3.] The JOB_POPULATION_DWH_RESOURCES_NOTIF job was created.');
