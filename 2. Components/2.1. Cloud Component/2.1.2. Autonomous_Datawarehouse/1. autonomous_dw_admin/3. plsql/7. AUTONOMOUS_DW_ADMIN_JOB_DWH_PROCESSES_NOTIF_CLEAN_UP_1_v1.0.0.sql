-- AUTONOMOUS_DW_ADMIN_JOB_DWH_PROCESSES_NOTIF_CLEAN_UP_1_v1.0.0
--"CLEAN UP JOB FOR DWH_PROCESSES_NOTIF TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
WHERE job_name = 'JOB_CLEAN_UP_DWH_PROCESSES_NOTIF';
IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB('JOB_CLEAN_UP_DWH_PROCESSES_NOTIF', FORCE => TRUE);
END IF;
...
BEGIN
  DBMS_SCHEDULER.create_job (
    job_name        => 'JOB_CLEAN_UP_DWH_PROCESSES_NOTIF',
...
BEGIN
  ...
  WHERE creation_date < ADD_MONTHS(SYSTIMESTAMP, -1);
------------------------------------------------------
...
(process_name, 
 ...
 'DWH_PROCESSES_NOTIF_CLEAN_UP_PROCESS', 
...
-- 4. Cleanup logic 
-------------------- 
DELETE FROM autonomous_dw_tech_owner.dwh_processes_notif 
WHERE creation_date < ADD_MONTHS(SYSTIMESTAMP, -1); 
...
END;
    ...
    enabled         => TRUE,
    comments        => 'Deletes records older than 1 month from DWH_PROCESSES_NOTIF table;'
  );
END;
]';
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The JOB_CLEAN_UP_DWH_PROCESSES_NOTIF job wasnt created properly.');
END IF;

DBMS_OUTPUT.PUT_LINE('[3.] The JOB_CLEAN_UP_DWH_PROCESSES_NOTIF job was created.');
