...
--DELETE TABLE USER_SKILL IF EXIST;
SELECT COUNT(*) INTO v_count
FROM all_tables
...
v_sql := q'[
        CREATE TABLE autonomous_db_owner.user_skill (
          ...
          CONSTRAINT pk_user_skill_id PRIMARY KEY (user_id, skill_code),
          CONSTRAINT fk_user_id FOREIGN KEY (user_id) REFERENCES utilizatori (user_id),
          CONSTRAINT fk_skill_code FOREIGN KEY (skill_code) REFERENCES technology_skill (skill_code)
        )
    ]';
...
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The USER_SKILL table was created.');
--CREATE COMMENTS FOR TABLE AND COLUMNS
-- TABLE COMMENT
EXECUTE IMMEDIATE 'COMMENT ON TABLE autonomous_db_owner.user_skill IS ''The table contains all the links between a user and the skill that he knows''';

-- COLUMNS COMMENT
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_db_owner.user_skill.user_id IS ''The id of the user''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_db_owner.user_skill.skill_code IS ''The code of the technology skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_db_owner.user_skill.proficiency_level IS ''The proficiency level of the technology skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_db_owner.user_skill.experience_months IS ''The number of months when student used the technology skill''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN autonomous_db_owner.user_skill.last_used_date IS ''The last date when the student used the technology skill''';
...
--CREATE SEQUENCE SEQ_USER_SKILL_ID FOR PRIMARY KEY
--DELETE SEQUENCE IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_sequences
WHERE sequence_name = 'SEQ_USER_SKILL_ID';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP SEQUENCE autonomous_db_owner.seq_user_skill_id';
END IF;
--CREATE SEQUENCE
 EXECUTE IMMEDIATE '
    CREATE SEQUENCE seq_user_skill_id
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE
...
WHERE sequence_name = 'SEQ_USER_SKILL_ID';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The SEQ_USER_SKILL_ID sequence wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The SEQ_USER_SKILL_ID sequence for primary key was created.');

--CREATE TRIGGER FOR PRIMARY KEY;
--DELETE TRIGGER IF EXISTS;
SELECT COUNT(*) INTO v_count
FROM user_triggers
WHERE trigger_name = 'TRG_USER_SKILL_ID_PK';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER autonomous_db_owner.trg_user_skill_id_pk';
END IF;
--CREATE TRIGGER
v_sql := 'CREATE OR REPLACE TRIGGER trg_user_skill_id_pk
          BEFORE INSERT ON autonomous_db_owner.user_skill
          FOR EACH ROW
          WHEN (NEW.user_skill_id IS NULL)
          BEGIN
             SELECT seq_user_skill_id_id.NEXTVAL INTO :NEW.user_skill_id_id FROM dual;
          END;';          
EXECUTE IMMEDIATE v_sql;
...
WHERE trigger_name = 'TRG_USER_SKILL_ID_PK';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The TRG_USER_SKILL_ID_PK trigger wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[4.] The TRG_USER_SKILL_ID_PK trigger for primary key was created.');
...
END IF;
DBMS_OUTPUT.PUT_LINE('[5.] The TRG_USER_SKILL_TECH_COL trigger for technical columns was created.');
