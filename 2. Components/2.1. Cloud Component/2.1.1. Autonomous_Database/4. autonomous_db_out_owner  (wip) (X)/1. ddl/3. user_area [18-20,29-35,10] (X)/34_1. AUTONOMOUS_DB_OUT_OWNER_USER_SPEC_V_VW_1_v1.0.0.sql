--AUTONOMOUS_DB_OUT_OWNER_USER_SPEC_V_VW_1_v1.0.0
--"USER_SPEC_V VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'USER_SPEC_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.user_spec_v CASCADE CONSTRAINTS';
END IF;
--CREATE USER_SPEC TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.user_spec_v AS
SELECT 
specialization_id,
user_id,
graduation_date,
...
AND view_name = 'USER_SPEC_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The USER_SPEC_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[3.] The USER_SPEC_V view was created.');
