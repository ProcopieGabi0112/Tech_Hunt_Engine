-- AUTONOMOUS_DB_ADMIN_JOB_CONNECT_NOTIF_CLEAN_UP_1_v1.0.0
--"CLEAN UP JOB FOR CONNECT_NOTIF TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
WHERE job_name = 'JOB_CLEAN_UP_CONNECT_NOTIF';
IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB('JOB_CLEAN_UP_CONNECT_NOTIF', FORCE => TRUE);
END IF;
...
BEGIN
  DBMS_SCHEDULER.create_job (
    job_name        => 'JOB_CLEAN_UP_CONNECT_NOTIF',
    ...
    job_action      => q'{
      ...
      BEGIN
        ...
        FROM autonomous_db_tech_owner.connect_notif 
        ...
        (process_name, 
         ...
         -- 4. Cleanup logic 
         -------------------- 
         DELETE FROM autonomous_db_tech_owner.connect_notif 
         WHERE creation_date < SYSTIMESTAMP - INTERVAL '7' DAY;
...
END;
    ...
    enabled         => TRUE,
    comments        => 'Deletes records older than 1 week from CONNECT_NOTIF table'
  );
END;
]';
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The JOB_CLEAN_UP_CONNECT_NOTIF job wasnt created properly.');
END IF;

DBMS_OUTPUT.PUT_LINE('[3.] The JOB_CLEAN_UP_CONNECT_NOTIF cleanup job was created.');
