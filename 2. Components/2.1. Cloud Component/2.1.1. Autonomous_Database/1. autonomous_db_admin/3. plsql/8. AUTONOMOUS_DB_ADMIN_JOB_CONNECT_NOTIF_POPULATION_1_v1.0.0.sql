-- AUTONOMOUS_DB_ADMIN_JOB_CONNECT_NOTIF_POPULATION_1_v1.0.0
--"POPULATION JOB FOR CONNECT_NOTIF TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
WHERE job_name = 'JOB_POPULATION_CONNECT_NOTIF';
IF v_count > 0 THEN
    DBMS_SCHEDULER.DROP_JOB('JOB_POPULATION_CONNECT_NOTIF', FORCE => TRUE);
END IF;
...
BEGIN
  DBMS_SCHEDULER.CREATE_JOB (
    job_name        => 'JOB_POPULATION_CONNECT_NOTIF',
    ...
    job_action      => q'!
      ...
      BEGIN
        ...
        FOR j IN 1 .. v_sessions.COUNT LOOP
          ...
          IF v_count = 0 THEN
            INSERT INTO autonomous_db_tech_owner.connect_notif (
              conn_key,
              user_name,
          ...
          ELSIF v_sessions(j).status = 'INACTIVE' THEN
            UPDATE autonomous_db_tech_owner.connect_notif
...
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The JOB_POPULATION_CONNECT_NOTIF job wasnt created properly.');
END IF;

DBMS_OUTPUT.PUT_LINE('[3.] The JOB_POPULATION_CONNECT_NOTIF job was created.');
