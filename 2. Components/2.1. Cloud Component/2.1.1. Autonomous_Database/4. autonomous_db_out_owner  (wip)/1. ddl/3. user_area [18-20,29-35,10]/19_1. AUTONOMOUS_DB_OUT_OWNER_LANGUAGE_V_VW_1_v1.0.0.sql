--AUTONOMOUS_DB_OUT_OWNER_LANGUAGE_V_VW_1_v1.0.0
--"LANGUAGE VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'LANGUAGE_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.language_v CASCADE CONSTRAINTS';
END IF;
--CREATE TECHNOLOGY_TYPE TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.language_v AS
SELECT 
lang_code,
name,
iso_code,
...
last_synced_at,
deleted_flag
FROM autonomous_db_owner.language;
]';
...
AND view_name = 'LANGUAGE_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The LANGUAGE_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The LANGUAGE_V view was created.');
