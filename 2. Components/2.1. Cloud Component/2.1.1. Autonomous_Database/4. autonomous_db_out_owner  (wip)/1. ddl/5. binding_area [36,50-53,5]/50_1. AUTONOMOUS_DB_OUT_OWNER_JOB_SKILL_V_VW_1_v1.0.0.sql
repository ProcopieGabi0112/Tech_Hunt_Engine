--AUTONOMOUS_DB_OUT_OWNER_JOB_SKILL_V_VW_1_v1.0.0
--"JOB_SKILL_V VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
--DELETE TABLE JOB_SKILL_V IF EXIST;
SELECT COUNT(*) INTO v_count
FROM all_views
WHERE owner = 'AUTONOMOUS_DB_OUT_OWNER'
AND view_name = 'JOB_SKILL_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.job_skill_v CASCADE CONSTRAINTS';
END IF;
--CREATE JOB_SKILL_V TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.job_skill_v AS
SELECT 
skill_code,
job_id,
required_level,
...
last_synced_at,
deleted_flag
FROM autonomous_db_owner.job_skill;
]';
