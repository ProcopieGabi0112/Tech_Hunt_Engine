--AUTONOMOUS_DB_OUT_OWNER_ROLE_V_VW_1_v1.0.0
--"ROLE_V VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'ROLE_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.role_v CASCADE CONSTRAINTS';
END IF;
--CREATE TECHNOLOGY_TYPE TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.role_v AS
SELECT 
role_id,
name,
creation_date,
...
AND view_name = 'ROLE_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The ROLE_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The ROLE_V view was created.');
