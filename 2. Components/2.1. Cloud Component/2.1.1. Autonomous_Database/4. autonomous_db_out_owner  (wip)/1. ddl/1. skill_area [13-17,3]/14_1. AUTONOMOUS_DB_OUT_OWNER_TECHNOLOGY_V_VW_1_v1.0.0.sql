--AUTONOMOUS_DB_OUT_OWNER_TBL_TECHNOLOGY_V_VW_1_v1.0.0
--"TECHNOLOGY_V VIEW"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
AND view_name = 'TECHNOLOGY_V';
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP VIEW autonomous_db_out_owner.technology_v CASCADE CONSTRAINTS';
END IF;
--CREATE TECHNOLOGY_TYPE TABLE;
v_sql := q'[
CREATE OR REPLACE VIEW autonomous_db_out_owner.technology_v AS
SELECT 
technology_code,
name,
release_date,
...
AND view_name = 'TECHNOLOGY_V';
IF v_count = 0 THEN
    RAISE_APPLICATION_ERROR(-20001,'The TECHNOLOGY_V view wasnt created properly.');
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The TECHNOLOGY_V view was created.');
