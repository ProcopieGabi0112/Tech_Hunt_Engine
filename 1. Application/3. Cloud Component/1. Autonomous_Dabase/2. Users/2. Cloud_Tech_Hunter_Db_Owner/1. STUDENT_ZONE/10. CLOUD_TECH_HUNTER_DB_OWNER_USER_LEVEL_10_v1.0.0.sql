--CLOUD_TECH_HUNTER_DB_OWNER_USER_LEVEL_10_v1.0.0
--"USER_ROLE TABLE"
SET SERVEROUTPUT ON;
DECLARE
  v_count NUMBER;
...
--DELETE TABLE USER_LEVEL IF EXIST;
...
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TABLE tech_hunter_db_owner.user_level CASCADE CONSTRAINTS';
END IF;
...
END IF;
DBMS_OUTPUT.PUT_LINE('[2.] The USER_LEVEL table was created.');
...
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.created_by IS ''Technical Column - The user who created the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.last_update_date IS ''Technical Column - The last update date of the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.last_updated_by IS ''Technical Column - The user who updated the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.source_system IS ''Technical Column - The source system of the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.sync_status IS ''Technical Column - The sync status of the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.sync_version IS ''Technical Column - The sync version of the record''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.last_synced_at IS ''Technical Column - The date when the record was last time synced''';
EXECUTE IMMEDIATE 'COMMENT ON COLUMN tech_hunter_db_owner.user_level.deleted_flag IS ''Technical Column - The flag indicating if the record is deleted or not''';
...
IF v_count > 0 THEN
    EXECUTE IMMEDIATE 'DROP TRIGGER tech_hunter_db_owner.trg_user_level_tech_col';
END IF;
--CREATE TRIGGER
v_sql := '  CREATE OR REPLACE TRIGGER trg_user_level_tech_col
            BEFORE INSERT OR UPDATE ON tech_hunter_db_owner.user_level
            FOR EACH ROW
            BEGIN
                 IF INSERTING THEN
                    :NEW.created_by := USER;
